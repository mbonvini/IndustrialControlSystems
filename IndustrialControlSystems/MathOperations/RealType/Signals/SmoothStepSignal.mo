...
model SmoothStepSignal "Smooth step Set Point generator, driven by signals"
...
equation
  En_d = ENup;
  when
      (pre(En_d)== false and En_d==true) then
    StartTime = time;
    St = true;
  end when;
  y = if
        (St==false) then
                        yin else Functions.fSmoothStep(time,StartTime+delay,alfa,m,yin,yfin);
  ...
  <br><br>
  <img src=\"modelica://IndustrialControlSystems/help/images/Math/RealType/Signals/SmoothStepSignal.png\">
  <br><br>
  ...
  In this case <FONT FACE=Courier>alfa = 0.1</FONT>, this indicates that each junction occupy the 10 % of the total rising time.<br>
  The additional parameter <FONT FACE=Courier>Delay = 1.5</FONT>, indicates that the output has to be delayed by <FONT FACE=Courier>1.5</FONT> seconds. 
    </p>
