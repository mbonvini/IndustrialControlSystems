...
model TestPID_TDOe
  ...
  PID_2dof_TDO_FullyEventBased ControllerTDOe(
    AntiWindup=true,
    CSmin=0,
    CSmax=2,
    Ti=2,
  ...
  LinearSystems.Continuous.TransferFunction ProcessTDOe(num={5,1}, den={20,12,1})
  ...
  Modelica.Blocks.Math.Feedback sub1
        ...
        origin={0,-30})));
equation
  connect(step.y, ControllerTDOe.SP)
                           annotation (Line(
      points={{-79,80},{-76,80},{-76,38},{-38,38}},
      color={0,0,127},
      smooth=Smooth.None));

  connect(ControllerTDOe.CS, sub.u1)
                                 annotation (Line(
      points={{-21,30},{-8,30}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(sub.y, ProcessTDOe.u)
                            annotation (Line(
      points={{9,30},{20.4,30}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(disturb.y, sub.u2) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(ProcessTDOe.y, ControllerTDOe.PV)
                                    annotation (Line(
  ...
  <p>
  The event based implementation of the digital incremental PID with TDO, reduces the time spent for simulating the model. 
  Here follows the simulation results for the considere examples
  <pre>
