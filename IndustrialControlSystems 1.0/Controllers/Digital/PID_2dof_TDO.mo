...
algorithm
  when sample(0,Ts/TDsteps) then
    ...
    if counter>=Ts then
      ...
      cso     := cs;
      dold    := d;
      StepsUp :=((cs-CSmin)/(CSmax-CSmin)*Ts);

    end if;

    CS := if (counter<StepsUp) then CSmax else CSmin;
       ...
       annotation (Diagram(graphics),         Icon(graphics={Text(
          ...
          fillColor={213,255,170},
          fillPattern=FillPattern.Solid,
          textString="TDO")}),Documentation(info="
  <HTML>
  <h4>Description</h4>
  <p>
  ...
  The sampling time <FONT FACE=Courier>Ts</FONT> is divided in <FONT FACE=Courier>TDsteps</FONT> intervals. For each one of these sub intervals, <br>
  the CS may be at <FONT FACE=Courier>CSmin</FONT> or <FONT FACE=Courier>CSmax</FONT>. This results in a very high frequency sampling that may slow down the simulation.
